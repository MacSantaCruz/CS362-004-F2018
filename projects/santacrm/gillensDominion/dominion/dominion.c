...
int adventurer_run(int currentPlayer, struct gameState *state, int handPos){
  int drawntreasure = 0;
...
int great_hall_run(int currentPlayer, struct gameState *state, int handPos){
  //+1 Card
  drawCard(currentPlayer, state);
...
int village_run(int currentPlayer, struct gameState *state, int handPos){
  //+1 Card
  drawCard(currentPlayer, state);
  ...
  //BUG - commented out below
  //discardCard(handPos, currentPlayer, state, 0);
  return 0;
}

int steward_run(int currentPlayer, struct gameState *state, int handPos, int choice1, int choice2, int choice3){
  if (choice1 == 1)
  {
    //+2 cards
    drawCard(currentPlayer, state);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
  {
    ...
    // )
    case adventurer:
      return adventurer_run(currentPlayer, state, handPos);
    ...
    case smithy:
      return smithy_run(currentPlayer, state, handPos);

    case village:
      return village_run(currentPlayer, state, handPos);
    ...
    case great_hall:
      great_hall_run(currentPlayer, state, handPos);
    ...
    case steward:
      steward_run(currentPlayer, state, handPos, choice1, choice2, choice3);
